...
const exams = {
    ...
    Hiring: {
        ...
        instructions: `For each question, select the ONE answer that best describes your organization's current state.\n\nAll questions are required.\n\nIf unsure, choose closest to reality (not aspiration).`,
        questions: [
            {
...
/* -----------------------------
   SIDEBAR SUB-NAV HELPERS
------------------------------ */
function showDiagnosticSubNav() {
    document.getElementById("diagnosticSubNav").style.display = "block";
    diagnosticActive = true;
}

function hideDiagnosticSubNav() {
    document.getElementById("diagnosticSubNav").style.display = "none";
    diagnosticActive = false;
}

function setActiveSubBtn(type) {
    document.querySelectorAll(".sub-nav-btn").forEach(btn => btn.classList.remove("active-sub"));
    if (type) {
        const target = document.getElementById("sub" + type);
...
/* -----------------------------
   PAGE SYSTEM
------------------------------ */
// IDs of all top-level sidebar buttons and which page they map to
...
function clearSidebarActive() {
    Object.keys(sidebarPageMap).forEach(btnId => {
        const btn = document.getElementById(btnId);
        if (btn) btn.classList.remove("active-sidebar");
    });
    // also clear logout highlight if any
    const logoutBtn = document.getElementById("logoutBtn");
    if (logoutBtn) logoutBtn.classList.remove("active-sidebar");
}
...
function showPage(id) {
    ...
    // hide sub-nav unless we're going to diagnostic
    if (id !== "diagnostic") {
        hideDiagnosticSubNav();
        setActiveSubBtn(null);
    }

    // activate the matching sidebar button
    const matchingBtn = Object.keys(sidebarPageMap).find(k => sidebarPageMap[k] === id);
    if (matchingBtn) {
        document.getElementById(matchingBtn).classList.add("active-sidebar");
    }

    if (id === "diagnostic") {
        showDiagnosticSubNav();
        renderDashboard();
    }
...
/* -----------------------------
   START EXAM (from content grid tiles)
------------------------------ */
...
/* -----------------------------
   START EXAM (from sidebar sub-nav)
------------------------------ */
...
function loadQuestion() {
    ...
    document.getElementById("questionBox").innerHTML = `
        ...
        <div class="nav-buttons ${questionIndex === 0 ? 'single-next' : ''}">
            ${questionIndex > 0 ? `
                <button class="primary nav-btn-small" onclick="prevQuestion()">Back</button>
            ` : ""}
...
function finishExamFromAnswers() {
    ...
    let total = 0;
    answers.forEach(val => { if (val !== null) total += val; });
